//
//  AppImages.swift
//  S-Forum
//
//  Created by Fakunabs on 18/10/2023.
//

import UIKit

struct AppImages {
    static let selectedTabIcon = UIImage(named: "selected-tab-icon")
    static let unselectedTabIcon = UIImage(named: "unselected-tab-icon")
    static let groupTabbarIcon = UIImage(named: "group-tabbar-icon")
    static let homeTabbarIcon = UIImage(named: "home-tabbar-icon")
    static let scheduleTabbarIcon = UIImage(named: "schedule-tabbar-icon")
    static let podcastTabbarIcon = UIImage(named: "podcast-tabbar-icon")
    static let interviewTabbarIcon = UIImage(named: "interview-tabbar-icon")
    static let homeFollowingIcon = UIImage(named: "home-following-icon")
    static let homeNewIcon = UIImage(named: "home-new-icon")
    static let homePopularIcon = UIImage(named: "home-popular-icon")
    static let sgroupLogoIcon = UIImage(named: "sgroup-logo-icon")
    static let homeSelected = UIImage(named: "home-selected")
    static let calendarSelected = UIImage(named: "calendar-selected")
    static let tempImage1 = UIImage(named: "temp-image")
    static let tempImage2 = UIImage(named: "temp-image2")
    static let tempImage3 = UIImage(named: "temp-image3")
}
